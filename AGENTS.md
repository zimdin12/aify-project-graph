...
**Profile:** With no `--toolset`/`AIFY_GRAPH_PROFILE`, Claude Code now gets the focused **`default`** surface (~15 intent verbs listed; the rest stay callable by name via `tools/call`). `--toolset=full` lists the whole 30-verb API. Codex and OpenCode use `--toolset=lean` (6 visible: `graph_packet`, `graph_consequences`, `graph_pull`, `graph_change_plan`, `graph_health`, `graph_watch`; the other verbs remain callable by name). `APG_MCP_TOOLS=<csv>` restricts the *listed* set further for A/B ablation (still callable). Every verb stays invokable regardless of profile — gating is listing-only.
