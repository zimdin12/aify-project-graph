...
Measured on **apg's verified-fresh postfix4 self-bench** (2026-04-25): mixed-mode graph use (briefs first, live verbs for precision, source for proof) beat no-graph **6-2**, saved **−17.3% net tokens**, tightened quality delta to **−0.25**, and ended in **trust ok** state. Exact artifact: [docs/dogfood/token-cost-bench-2026-04-25-postfix4.json](docs/dogfood/token-cost-bench-2026-04-25-postfix4.json). Older 2026-04-20 cross-runtime data still gives context: Claude Code Agent + Opus saw **−19% to −34% tokens and 1.5-2.9× wall-clock** on shell-accessible orient/search tasks, while Codex + gpt-5.4 was roughly parity aggregate on those older pre-fix shapes. Treat the new headline as **post-fix apg-only**; broader cross-runtime / larger-scale validation is pending a separate round.
