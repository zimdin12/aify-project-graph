...
**Profile:** Claude Code uses the full callable surface, with a few legacy orient aliases hidden from `tools/list`; Codex and OpenCode use `--toolset=lean` (3 visible: `graph_consequences`, `graph_pull`, `graph_change_plan`; the other verbs remain callable by name via `tools/call`).
...
Measured on **apg's verified-fresh post-upgrade self-bench** (2026-04-26): mixed-mode graph use (packet for orientation, change_plan/consequences for depth, source for proof) beat no-graph **6-2**, saved **−23.1% net tokens** (improved from −17.3% pre-upgrade), with quality delta **−0.50**. Trust gate `ok`. Exact artifact: [docs/dogfood/token-cost-bench-2026-04-26-final.json](docs/dogfood/token-cost-bench-2026-04-26-final.json). 2026-04-25 upgrade plan (M0.5 + M1 + M2 + M3 + M4a + M4b) details at [docs/superpowers/plans/2026-04-25-upgrade-plan.md](docs/superpowers/plans/2026-04-25-upgrade-plan.md). Older 2026-04-20 cross-runtime data: Claude Code Agent + Opus saw **−19% to −34% tokens and 1.5-2.9× wall-clock**, Codex + gpt-5.4 roughly parity. Broader cross-runtime / larger-scale validation pending a separate round.
