...
**READING `evidence`: diagnose with `cause`, and ignore the two booleans.** `degraded` is true on essentially every answer (a standing cause is always set), so it does not discriminate and cannot tell you anything. `operationallyDegraded` is a second overlapping boolean and is no better. Both are **deprecated** and will be removed. ⛔ **NO FIELD LICENSES AN ABSENCE CLAIM, so there is nothing to branch on for one.** `exhaustive` is withheld on every verb, and on 2026-09-06 `capabilities.absenceAuthority` stopped being grantable for the same reason — the compile DB never reports which TUs clangd actually indexed, so no coverage number establishes the population the claim needs. For "is this dead / safe to delete", `rg` is the instrument. What the evidence contract is for is knowing when to REFUSE: `cause` names what limited the answer so you can act on it — a specific cause like `translation_unit_did_not_compile` is actionable, the standing `index_population_unattested` is true of every call.

`code_intel_references` and `code_intel_hierarchy` accept **`acceptEvidenceContractVersion`**. Omit it and you get contract 1, which still carries the deprecated booleans — old callers are never broken. Pass `2` to receive the contract with both booleans removed. An unsupported value is **refused, never quietly downgraded**, so you can never end up believing you hold a contract that was not emitted. Every response echoes the `contractVersion` it was rendered as; check it rather than inferring it.
...
**Compile-DB coverage gate (read the evidence, not just the result).** ⛔ **SUPERSEDED 2026-08-19: they do not attest an exhaustive caller set at all, and the sentence below described a condition we never verified.** The compile DB says which TUs clangd MAY index; it never reports which it DID. A TU present in the DB can still fail to compile — a missing include path is enough — while background indexing reports idle, so `exhaustive` is now withheld with cause `index_population_unattested`. Returned locations remain compiler-resolved and precise; the SET is a floor, and absence claims need rg. The toolchain/unity guidance below is still correct as far as it goes: When the `compile_commands.json` was built by a *different* toolchain than the host clangd (e.g. a Linux/WSL DB with `/mnt/c` paths run against Windows clangd) or is a CMake **unity/jumbo** build, the index is silently PARTIAL — some real callers are invisible. These verbs now detect that and return `evidence.degraded:true, cause:"partial_compile_db_coverage", exhaustive:false` with an "verify with rg before delete/rename" fallback **instead of a false `exhaustive:true`**. When you see that cause: do NOT treat the result as "no callers / dead code / safe to delete" — cross-check with Grep. To make the index actually complete, generate a NATIVE Windows compile DB so host clangd matches it. MSBuild does NOT emit `compile_commands.json`, so configure a Ninja+clang-cl build purely for the DB: `cmake -S . -B build-clangd -G Ninja -DCMAKE_C_COMPILER=clang-cl -DCMAKE_CXX_COMPILER=clang-cl -DCMAKE_EXPORT_COMPILE_COMMANDS=ON`. **The directory name does not matter** — as of 2026-08-25 candidates are DERIVED from disk, so any dir holding a `compile_commands.json` is found. ⛔ **THEN VERIFY WHAT LANDED IN IT.** A configure can silently fall back to MSVC: in sand_castle a `build-win-clangd/` existed exactly as our own advice instructed and contained 679/679 `cl.exe`, and the selector preferred it BY NAME over a real clang-cl DB beside it. Check that each entry's command names `clang-cl`, not `cl.exe`; a mismatch now returns `toolchain_mismatch` and names the better DB if one is on disk. ⛔ **AND THE clangd PROCESS ITSELF NEEDS THE MSVC ENVIRONMENT.** Generating the DB without a vcvars shell is fine; CONSUMING it is not. A clangd with no `INCLUDE` returns an EMPTY caller set for any TU including a standard header — measured, 2 refs vs 0 on identical TUs differing only by `#include <cstddef>`. That now surfaces as `evidence.translationUnitFailed` with the unresolved header named; the fix is to launch the MCP server from a Developer Command Prompt so `INCLUDE` is inherited. `APG_CLANGD_WSL=1` (clangd under WSL against a Linux DB) is the fallback if a WSL build is still around.
...
**⛔ `evidence.degraded` NO LONGER DISCRIMINATES — read `cause`.** As of 2026-08-25 both `code_intel_references` and `code_intel_hierarchy` return `degraded: true` on EVERY answer, with the standing cause `index_population_unattested`: the compile DB reports which TUs clangd MAY index, never which it DID, and a TU in it can still fail to compile while indexing reports idle. So `degraded` is now true of every call and carries no information — branch on `cause` instead. The causes that mean something specific went wrong are `translation_unit_did_not_compile` (that TU has no AST; the separate field `evidence.translationUnitFailed` is set and `evidence.missingHeaders` names the unresolved header), `no_incoming_unconfirmed`, `truncated_to_caps`, `cold_index`, `definition_only`, `stale_index`, `timeout`. ⚠ `bounded_mode` is NOT one of them — it is selected behaviour, not an incident, and no longer marks a session degraded. ⚠ And `translationUnitFailed` is a FIELD, not a cause value; an earlier version of this line named the field among the causes, so branching on it as a `cause` would never have matched.
