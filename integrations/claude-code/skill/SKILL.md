...
**READING `evidence`: branch on `exhaustive === true`, diagnose with `cause`, and ignore the two booleans.** `degraded` is true on essentially every answer (a standing cause is always set), so it does not discriminate and cannot tell you anything. `operationallyDegraded` is a second overlapping boolean and is no better. Both are **deprecated** and will be removed. The only field that licenses an absence claim ("no callers", "safe to delete") is `exhaustive === true`; `cause` names what limited the answer so you can act on it — a specific cause like `translation_unit_did_not_compile` is actionable, the standing `index_population_unattested` is true of every call.
