---
name: graph-setup
description: Use when the user says "generate project graphs", "build graph for this repo", "index this repo", "set up aify-project-graph here", or opens a new repo that has no `.aify-graph/` directory yet. Runs the full first-time setup in one pass — graph index, all five briefs, and a proposed functionality.json. User only needs to review the functionality diff.
---

# graph-setup

Build the complete graph + brief + overlay set for a repo in one go. This is the skill that answers "just get me set up" without six manual commands.
...
Run the `graph-map-functionality` skill's logic inline: read `.aify-graph/brief.json`, draft a set of 5-10 features with `anchors.symbols` and `anchors.files` globs, show the user a diff, write `.aify-graph/functionality.json` on confirmation.
...
If the user has a task tracker (ClickUp/Asana/Linear/Jira/GitHub) connected via MCP, offer to run `/graph-map-tasks` to add `tasks.json`. Skip if no task source is detected — this is optional enrichment.
...
- "Run `/graph-setup` again whenever the repo changes significantly or you want to refresh the overlay."
