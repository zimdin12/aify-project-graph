...
**Bounded live code-intel verbs (2026-05-12 — for C++ inner-loop editing).** When you're mid-edit and need ONE bounded answer (diagnostics for these files, callers of this symbol, type at this position), use the bounded `code_intel_*` verbs FIRST — they drive clangd live, no collect/import cycle, return 5-12× less data than `graph_collect_code_intel`+`graph_pull`:
