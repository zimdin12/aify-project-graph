...
Measured on **apg's verified-fresh self-bench post upgrade-plan v2** (2026-04-26): graph-enabled mixed mode won **6-2**, cut net tokens by **−23.1%** (improved from −17.3% pre-upgrade), with quality delta **−0.50** (one residual: `graph_packet`'s coarse view trades depth for cost on PLAN/IMPACT — escalate to `graph_change_plan`/`graph_consequences` when the symbol is load-bearing). Trust gate ends `ok`. Exact artifact: [docs/dogfood/token-cost-bench-2026-04-26-final.json](docs/dogfood/token-cost-bench-2026-04-26-final.json). The upgrade plan introduced `graph_packet` (one-shot agent prompt packet, overlay-first, no SQL), compact Codex SKILL card (3800→780 tokens), latency surgery, and brief honesty signals (SNAPSHOT line, scratch-vs-source dirty grouping, feature truncation indicator). Earlier 2026-04-20 cross-runtime data: Claude Code Agent + Opus saw **1.5–2.9× faster wall-clock** and **−19% to −34% tokens** on shell-accessible orient/search/trace tasks, while Codex + gpt-5.4 was roughly parity aggregate on those older pre-fix shapes — kept as cross-runtime context, but the post-upgrade headline is the current measurement.
...
**Measured on apg's verified-fresh post-upgrade self-bench (2026-04-26):** mixed-mode graph usage (briefs first, packet for orientation, change_plan/consequences for depth, source for proof) beat no-graph **6-2**, saved **−23.1% net tokens** (improved from −17.3% in the pre-upgrade postfix4 baseline), with quality delta **−0.50** (one residual: packet trades depth for cost on PLAN/IMPACT — escalate to depth-verbs when load-bearing). Trust gate `ok`. Exact artifact: [docs/dogfood/token-cost-bench-2026-04-26-final.json](docs/dogfood/token-cost-bench-2026-04-26-final.json). The 2026-04-25 upgrade shipped `graph_packet` (one-shot agent prompt packet, overlay-first, no SQL — see plan at [docs/superpowers/plans/2026-04-25-upgrade-plan.md](docs/superpowers/plans/2026-04-25-upgrade-plan.md)), compact Codex SKILL card, latency surgery, and brief honesty signals. Older 2026-04-20 cross-runtime data still gives context: Claude Code Agent + Opus saw **−19% to −34% tokens and 1.5-2.9× wall-clock**, Codex + gpt-5.4 roughly parity. Broader cross-runtime / scale validation pending a separate round on a non-shared target.
