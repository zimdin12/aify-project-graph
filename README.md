...
**New 2026-05-12: bounded `code_intel_*` verbs for C++ inner-loop editing.** Top-level MCP tools (`code_intel_diagnostics/references/definitions/hover/symbols/analyze`) drive clangd live or bounded analyzer/build checks without a collect/import round-trip. Single-fixture A/B against `graph_collect_code_intel` + `graph_pull`: **0.65× time, 0.12× bytes (88% byte reduction)** with provenance and three-state result rendering preserved. Use bounded verbs for atomic mid-edit questions; use `code_intel_analyze` when you need `clang-tidy` or compile-command syntax evidence beyond plain LSP; use `graph_packet({mode:"plan"|"verify"})` for planning and post-edit decisions. Analyzer/build evidence preserves `partial` / `not_collected` states, so missing compile entries are not rendered as a clean result. Artifacts: `docs/dogfood/ab-2026-05-12-bounded-vs-collect.{txt,json}`. Skill walkthrough: `/cpp-inner-loop`. Lean profile for C++ hosts: `--toolset=code-intel`.
