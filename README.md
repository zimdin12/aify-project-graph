...
Measured on **apg's verified-fresh postfix4 self-bench** (post-fix, 2026-04-25): graph-enabled mixed mode won **6-2**, cut net tokens by **−17.3%**, tightened quality delta to **−0.25**, and flipped trust to **ok** on the clean postfix4 snapshot. Exact artifact: [docs/dogfood/token-cost-bench-2026-04-25-postfix4.json](docs/dogfood/token-cost-bench-2026-04-25-postfix4.json). Earlier 2026-04-20 cross-runtime data still matters for context: Claude Code Agent + Opus saw **1.5–2.9× faster wall-clock** and **−19% to −34% tokens** on shell-accessible orient/search/trace tasks, while Codex + gpt-5.4 was roughly parity aggregate on those older pre-fix shapes. Treat the new headline as **apg-only post-fix**, with broader cross-runtime / larger-scale validation intentionally deferred to a separate round.
...
**Measured on apg's verified-fresh postfix4 self-bench (2026-04-25):** mixed-mode graph usage (briefs first, live verbs for precision, source for proof) beat no-graph **6-2**, saved **−17.3% net tokens**, and tightened quality delta to **−0.25** while ending the bench in **trust ok** state. Exact artifact: [docs/dogfood/token-cost-bench-2026-04-25-postfix4.json](docs/dogfood/token-cost-bench-2026-04-25-postfix4.json). We still keep the older 2026-04-20 cross-runtime matrix as context: Claude Code Agent + Opus saw **−19% to −34% tokens and 1.5-2.9× wall-clock** on shell-accessible tasks, while Codex + gpt-5.4 was roughly parity aggregate on those older pre-fix shapes. Read the new headline as **post-fix apg-only**; broader cross-runtime / larger-scale validation is pending a separate non-shared target.
