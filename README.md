...
**Bounded `code_intel_*` verbs for C++ — load-bearing on trustworthy absence claims.** Top-level MCP tools (`code_intel_diagnostics/references/definitions/hover/symbols/analyze`) drive clangd live, no collect/import round-trip. After Plan #14 (2026-05-21) they carry a structured **evidence contract**: `code_intel_references` returns `evidence: {ready, degraded, cause, confidence, fallback, exhaustive, warnings[]}`. **Only `evidence.exhaustive === true` is safe basis for absence claims** ("no callers", "dead code", "safe to delete"). Degraded causes (`cold_index|timeout|definition_only|stale_index|unsupported|unknown`) carry a structured `fallback` instruction so an agent can recover (pass `warmupFiles[]`, raise `waitForReadyMs`, fall back to grep) rather than guess. Cold sessions auto-prewarm a bounded ≤15-file set (same-dir + `compile_commands.json` siblings only). Sticky degraded state per session warns when the index quietly recovered after an earlier under-report.

**Measurement caveat:** the single-fixture A/B (`docs/dogfood/ab-2026-05-12-bounded-vs-collect.{txt,json}`) shows ~1.1× time, ~76% byte reduction — the **byte reduction is mechanically real**, the wall-clock parity is a fixture artifact (the demo bypasses production cold-server waits to measure tool-surface latency). On real clangd, bounded verbs pay a real first-call cold-warmup cost; subsequent calls in-session are fast. The agent-code-intel reference repo ran a 9-task T1-T9 A/B and validated empirically that this class of tool is "load-bearing on multi-level absence claims, marginal on grep-solvable cases" — APG hasn't run its own equivalent measurement on cpp yet; the contract above is the design intent pending validation.
