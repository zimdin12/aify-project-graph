...
>
> For C++ inner-loop editing, the modern path is the bounded live verbs — `code_intel_diagnostics`, `code_intel_references`, `code_intel_definitions`, `code_intel_hover`, `code_intel_symbols`. They drive clangd live, no collect/import round-trip, ~5-12× less response data than `graph_collect_code_intel` + `graph_pull`. Pair with `graph_packet({mode:"verify", files:[...]})` after edits. Templates for downstream-project `.lsp.json` / `.mcp.json`: see `docs/integrations/lsp.json.example` and `mcp.json.example`.
