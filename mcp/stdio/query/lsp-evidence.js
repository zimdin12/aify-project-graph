...
//
// ⛔ THE FIRST VERSION OF THIS CLAUSE SAID "invisible to BOTH tiers" AND THAT WAS WRONG.
// It was derived from the compile-database model and shipped without being observed. Measured on a
// purpose-built fixture (a call inside `#ifdef FEATURE_X`, with FEATURE_X undefined in the compile
// command, and an always-compiled call as the positive control):
//
//   driver -> visibleCall   conf=0.95 [lsp✓]   clangd resolved it
//   driver -> hiddenCall    conf=0.60          NO lsp marker — heuristic only
//
// clangd produced exactly ONE edge, so the clangd half held. But TREE-SITTER PARSES TEXT and never
// evaluates the preprocessor, so it reported the inactive-branch call as if it were live. The tiers
// do not share this blind spot — they fail in OPPOSITE directions, and that is the more useful
// thing to tell an agent: the heuristic set can contain calls that never compile, while the
// compiler-verified set omits them.
//
// ⚠ Pointing an agent at code_intel_references still does not escape the undercount half.
//
...
export function constructCoverageClause(language) {
  ...
  return ' NOT MODELLED: calls through function pointers or std::function, calls generated by macros,'
    + ' and conditional compilation — the heuristic tier does not evaluate #if/#ifdef so it counts'
    + ' calls in INACTIVE branches (overcount), while clangd compiles ONE configuration and omits'
    + ' them (undercount). The two tiers disagree in opposite directions here.'
