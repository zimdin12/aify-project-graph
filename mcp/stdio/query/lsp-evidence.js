...
//
// ⚠ AND THE LIMIT SURVIVES THE BEST TIER WE HAVE. clangd resolves ONE compile configuration, so a
// symbol used only inside an inactive `#ifdef` branch is invisible to clangd too. Pointing an agent
// at code_intel_references does not escape this one, which is exactly why it is worth saying.
//
...
export function constructCoverageClause(language) {
  ...
  return ' NOT MODELLED: calls through function pointers or std::function, calls generated by macros,'
    + ' and code excluded by conditional compilation — no #if/#ifdef is evaluated, and clangd resolves'
    + ' ONE compile configuration, so an inactive branch is invisible to BOTH tiers.'
