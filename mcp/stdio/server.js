...
const TOOLS = [
  ...
  },
  {
    name: 'graph_health',
    handler: graphHealth,
    description: 'Single-call "is the graph usable right now?" check. Aggregates indexed state, trust level, unresolved-edge count, staleness (indexed commit vs HEAD), and overlay validity into one summary string + structured fields. Use at session start instead of stringing graph_status + graph_index + brief.plan.md parsing. Also reports server.commit (the build ANSWERING you, captured at process start) and warns when the running process is older than the checkout. TWO DENOMINATORS ARE NOW PUBLISHED RATHER THAN IMPLIED: lspVerifiedPctOfCalls divides by VERIFIABLE AND IN-SCOPE calls only (see lspVerifiedPctDenominator) — edges no LSP backend can check (lspUnverifiableCalls, by language) and edges in files git does not track (lspOutOfScopeCalls) are excluded from both numerator and denominator, because an edge nobody could have verified is not an edge that failed verification; and trustBasis publishes the rule taking total unresolved edges down to the trust-relevant subset, broken out by exclusion reason so a small HONEST population is distinguishable from a hidden one. SCOPE: a trust-spine-empty verdict constrains GRAPH-backed answers (graph_callers, graph_impact, graph_pull, graph_consequences) only — it does NOT constrain the live verbs, which query the language server directly; for a delete decision read evidence.exhaustive on code_intel_references, not this summary.',
    schema: { type: 'object', properties: {}, additionalProperties: false },
  },
  {
    name: 'graph_packet',
  ...
  },
  {
    ...
    description: 'Cross-layer traversal: "what breaks if I touch X?" Input: symbol name OR file path. Output: contracts potentially affected, features touching this symbol, open tasks on those features, adjacent tests, last-touched git history, risk flags. READ field_provenance BEFORE ACTING: every field is labelled observed (derived from graph structure — callers, importers, documents_mentioning) or inferred (reached through the curated feature/task overlay — co_consumer_files, contracts_potentially_affected). An absent INFERRED entry is NOT evidence of absence, and overlay_age_days tells you how stale that overlay is. documents_mentioning is the OBSERVED counterpart to the curated contract list, ranked by distinct mentioned nodes — it catches binding documents the overlay never anchored. co_consumer_files carries {items,total,truncated,limit}: check truncated before treating it as the whole set. Also returns a portable RECEIPT — the claim plus its invalidation conditions (pinned repo/index/server commits, overlay content hash, worktree dirty hash), replayable by a teammate and REFUSING to validate if any pin drifted. Use BEFORE planning a non-trivial change — it produces the grounding set an editor-agent actually needs. Flagship verb for cross-cutting planning and pre-edit safety checks.',
    schema: {
      type: 'object',
      properties: {
