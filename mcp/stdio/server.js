...
const SHORT_DESCRIPTIONS = new Map([
  ...
  ['code_intel_references', 'Compiler-backed "who references this symbol", asked of the LIVE language server — the only verb whose answer can support a delete or rename decision. ★ READ evidence BEFORE CONCLUDING: exhaustive is WITHHELD (cause index_population_unattested) — the compile DB says which TUs clangd MAY index, never which it DID, so ZERO REFERENCES IS NOT EVIDENCE OF NO CALLERS. Locations are compiler-resolved and precise; the SET is a floor. Verify with rg before any delete/rename. On a cold session pass waitForReadyMs (e.g. 25000), or a CORRECT answer comes back unattested and cannot license a deletion.'],
