...
export const TOOLS = [
  ...
  },
  {
    ...
    description: 'Compiler-backed "who references this symbol", asked of the LIVE language server — the only verb whose answer can support a delete or rename decision. Bounded and budgeted. ★ READ evidence BEFORE CONCLUDING: exhaustive is WITHHELD (cause index_population_unattested) — the compile DB says which TUs clangd MAY index, never which it DID, and a TU in it can still fail to compile while indexing reports idle, so ZERO REFERENCES IS NOT EVIDENCE OF NO CALLERS. Locations are compiler-resolved and precise; the SET is a floor. An empty result whose TU failed carries evidence.translationUnitFailed. On one measured C++ project EVERY not-found result was definition_only and none were true absences, so treat degraded as uninformative rather than negative — check your own repo with graph_health.refsNotFoundBreakdown. Unaffected by graph staleness, because it asks the language server rather than the snapshot. ON A COLD SESSION pass waitForReadyMs (e.g. 25000) — otherwise a CORRECT answer comes back UNATTESTED (degraded:true, cause stale_index, exhaustive:false) and cannot license a deletion.',
    schema: {
      type: 'object',
      properties: {
        language: { type: 'string', description: 'Optional — auto-inferred from the file extension (.cpp/.h→cpp, .ts/.tsx/.js→typescript, .py→python); explicit value wins. Servers (clangd/typescript-language-server/pyright) are bundled; the host needs no LSP config.' },
  ...
  },
  {
    ...
    schema: {
      ...
      properties: {
        ...
        kind: { type: 'string', enum: ['auto', 'code', 'all'], default: 'auto', description: 'auto (default) = code + documents/configs/dirs, never unresolved stubs. code = code only. all = everything including stubs.' },
