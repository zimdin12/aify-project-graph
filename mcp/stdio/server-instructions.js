...
- evidence.exhaustive:false carries a cause naming WHY it is not a complete set: coverage_unknown (coverage for this query could not be PROVEN — the default is fail-closed, so silence is never read as proof), partial_compile_db_coverage (C++ foreign/unity compile DB, or a DB that covers none of your first-party code — see below), partial_tsconfig_scope (the TS file is outside its nearest tsconfig's project, so tsserver ran it in a loose inferred project), python_dynamic_dispatch (Python is never provably exhaustive), or truncated_to_caps (a call-hierarchy tree hit the breadth/total caps or only the first root of an overload set was walked — raise breadthCap/totalCap or verify). In every case the set is a FLOOR; confirm with code_intel_references / rg before any absence claim.
