...
| **⭐ THE COLLECT HAS A TIME BOUND, IT IS ON BY DEFAULT, AND IT ANNOUNCES ITSELF** | `mcp/stdio/code-intel/providers/cpp-clangd.js`: `DEFAULT_COLLECT_BUDGET_MS = 40000` total wall clock, `DEFAULT_INDEX_WAIT_MS = 90000` for clangd's background index to go idle before `references`, capped at `remainingBudget() − BUDGET_TAIL_RESERVE_MS` (3000). Under contention it stops early and returns what it has — **and it says so three ways**: envelope `status: 'partial'`, per-op `reason: 'budget_exhausted_index_warming'` / `'budget_exhausted_N_of_M_files'`, and `session.indexReady` / `indexWaitMs` / `indexWaitReason` with a `refsFoundSymbols`/`refsNotFoundSymbols` split whose comment exists so a not-found is never read as no-callers. **So this is NOT the silent-truncation class**: there nothing is recorded, here three things are. |
| **⛔ THE TRAP FOR THE NEXT PERSON, WHO WILL RUN THE SAME GREP** | `grep budgetEnabled` returns `:790` — `if (budgetEnabled && mode === 'indexed' && !indexReady && …) budgetExhausted = true` — which reads as **the truncation marker gated behind an optional parameter**, i.e. a silent short collect whenever no caller passes `budgetMs`. That is a P0 shaped exactly like the defect one would be hunting. It is wrong: `resolveCollectBudgetMs` at `:63`, two functions away, falls back to the 40s default, so `budgetEnabled` is true in the default case and the gate does not gate. **The correct-looking grep is the trap**, and it was one read short of being filed as a confirmed defect. |
| **⛔ SO THE DEFECT IS IN THE TEST, and it is a rule rather than a fix** | Both failing tests do `expect(['ok','partial']).toContain(full.status)` — accepting a truncated collect — and then assert on `verifiedEdgeCount`. The sibling negative control never reads `status` at all. **The test accepts the producer's warning and then measures the quantity the warning was about.** RULE: a test may not measure a collect the producer declared `partial`. |
| **⚠ THE RULE'S SECOND HALF, or the fix is worse than the bug** | A skip is a silence. Converting this into a skip means it skips under load, runs when quiet, and **if the collect ever becomes permanently `partial` through a real regression, the test skips forever — and a permanent skip is indistinguishable from a pass in a 4138-test summary.** Already live in this repo's own evidence: the suite line reads `4 skipped (4138)` and never names which four. So the skip must be NAMED and COUNTED, with repeated skipping treated as failure. The four were audited and are benign — three `it.skip` placeholders that print a skip reason when clangd is absent, one deliberate skip at `tests/unit/eval/regression-invariants.test.js:322` commented as a test that would fake-pass — but that is an audit true today, while the log is read every day. Naming skipped tests in `scripts/run-suite.mjs` is the structural half. (Second half stated by dashboard-manager; adopted.) |
| **⚠ UNVERIFIED, and not to be written up as though it were not** | **The envelope from the RED run was never captured.** That the collect actually returned `partial` with `indexWaitReason = 'index_wait_timeout'` is a mechanism that FITS the observation and nothing more — the same bar this project applied to a borrowed mechanism earlier the same day. What closes it: one LOADED run with the envelope logged. **Pre-registered refutation: if that envelope comes back `ok` rather than `partial`, this mechanism is dead and the product does have the silent-truncation defect.** That is the outcome nobody expects, which is why it is written before the result. |
| **⭐ THE SUITE SATURATES THE HOST BY ITSELF, which reframes everything above** | Measured during the quiet full run: 2 vitest processes, **CPU 96%**. A 514-file run pins this machine with its own worker pool, so the clangd test inside a full suite is ALWAYS contending — with 513 other test files — on any host. It passed alone in 3.55s on a host still carrying 151 resident processes; what it did not carry was the rest of the suite. **"Wait for a quiet host" could never have reproduced the environment it passed in.** External agent load added to a saturation already underway rather than creating it. |
| **✅ OUTCOME — branch A of a pre-registration written before the result existed** | The quiet full suite at `e0f3a8c6` went **GREEN**: 514 files, 4134 passed, `VITEST_EXIT=0`, 904s against 2147s for the RED run on byte-identical code. Same host, same suite parallelism, same commit; the only variable removed was another agent's back-to-back `npm test` runs. **So the suite's own worker pool is survivable and external agent load was the difference.** Branch B — failing again with no external load, which would have shown the suite's parallelism sufficient on its own — did not happen. ⚠ n=1, and one pass does not measure the margin: the same test took 3.55s alone against 33.3s loaded, so it now passes with unknown headroom. ⭐ Had branch B occurred, the isolated green would have been the POSITIVE CONTROL rather than a weak result — it establishes this code can produce 2+ verified edges in 3.55s, so a shared-run failure would have been about the environment and not the collect. A sample that cannot confirm a hypothesis can still be the control for the next one. (Framing from dashboard-manager; adopted.) |
