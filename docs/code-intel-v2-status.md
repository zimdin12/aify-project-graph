...
- **Freshness self-heal (field-report fix).** `graph_index` is now in the default tool surface; the central staleness warning reports commits-behind + the self-heal hint; opt-in `APG_AUTO_REINDEX=1` makes the dispatch refresh a behind-HEAD graph BEFORE the read handler runs; optional `scripts/install-graph-hook.mjs` installs a post-commit reindex hook (`scripts/reindex.mjs`). Addresses the 2026-06-01 the field fleet Sand Castle A/B: a stale graph was worse than none for managed workers who couldn't reindex. **Overlay-build gap (#3) is a per-repo data action — run `/graph-build-functionality` on the target repo; not tool code.** Spec/plan: `docs/superpowers/{specs,plans}/2026-06-01-graph-freshness-self-heal*`.
- Spec/plan: `docs/superpowers/specs/2026-06-01-agent-front-door-design.md`, `docs/superpowers/plans/2026-06-01-agent-front-door.md`.

_Current full suite: **1089 pass / 8 skipped, 0 failures** (2026-06-02), after two adversarial bug-hunt rounds over this round's work (dashboard rebuild, freshness self-heal, semantic layer). Earlier "915"/"1058" figures in this doc are dated snapshots._
...
## Multi-language LSP backends (2026-06-02) — SHIPPED
The clangd-only trust spine now covers **TypeScript/JavaScript** (typescript-language-server) and **Python** (pyright) behind the same provider boundary + evidence contract. Live `code_intel_*` verbs and `graph_collect_code_intel` work on TS/JS/Python repos; language is **inferred from the file extension / repo markers** (no `language` arg needed). Spec/plan: `docs/superpowers/{specs,plans}/2026-06-02-multi-language-lsp-backends*`.
- **Groundwork** (`code-intel/backends.js`, `coverage.js`, `node-bin.js`): a language→backend registry replaces the hardcoded `if (language==='cpp')`; a per-language coverage strategy replaces the cpp-only compile-DB gate.
- **Provisioning:** servers are **bundled as plugin npm deps** (`typescript-language-server`+`typescript`, `pyright`), resolved project-local→plugin→PATH and run via `node` (Windows `.cmd` EINVAL avoided). The MCP server owns the subprocess — **Claude Code / Hermes need no LSP config**. `apg code-intel doctor typescript|python` reports readiness.
- **Honesty per language:** C++ gated on compile-DB coverage; TS exhaustive **with** a `tsconfig.json` (partial without); **Python is never provably exhaustive** (duck typing / `getattr` / dynamic dispatch) → references/hierarchy return `exhaustive:false`, a verified floor.
- **Verified against the real servers:** cross-file references (TS exhaustive w/ tsconfig), symbols, call hierarchy, and scope=all collections with repo-relative records. +33 tests (unit + self-skipping real-server integration). Commits P0–P3 (`32185b1`→`e668f2b`) + repo-language detection (`5fdf52d`) + docs (`138f51b`).
